...
import { City } from '../../../../onboarding/models/city';
...
import { formatDateYYYYMMDD, yearsAgo, yearsAgoLegacy } from '../../../utils/datetime';
import { butonFunctionDis, buttonFunctionEn, formFunctionDis, formFunctionEnAll } from '../../../utils/disableOrEnabled';
...
import { minDateValidator, maxDateValidator } from '../../../utils/validators';
import moment from 'moment';
import { MatDatepicker, MatDatepickerInputEvent } from '@angular/material/datepicker';


@Component({
  selector: 'app-modal-ppe-family',
  standalone: false,
  templateUrl: './modal-ppe-family.component.html',
  styleUrl: './modal-ppe-family.component.scss'
})
export class ModalPpeFamilyComponent {
  ...
  constructor(private readonly modalRef: MatDialogRef<ModalPpeFamilyComponent>,
...
validationDataCurp(): string {
  ...
  validador(): boolean {
    ...
    if (this.profileForm.getRawValue().rfc?.trim() != '') {
      if (!REGEX.RFC_VALIDATION.test(this.profileForm.get('rfc')?.value) && this.profileForm.getRawValue().typeIden === "1" && !this.profileForm.get('curp')?.disabled) {
        const controlRfc = this.profileForm.get('rfc');
        controlRfc?.setErrors({ invalidFormat: true });
        controlRfc?.markAsTouched();
        this.error();
  ...
  async saveFppe() {
    ...
    if (resultDataPositionHeld == null) {
    return;
    }
    const data: DataRealOwnerClientFamilyPPE = this.client();
    this.curpAux = '';
