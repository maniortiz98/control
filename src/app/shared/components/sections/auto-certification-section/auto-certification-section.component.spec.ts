...
import { AutoCertificationSectionComponent } from './auto-certification-section.component';
...
/**
 * Specs de lógica para AutoCertificationSectionComponent (post-fixes).
...
function safeUUID(): string {
  ...
  // UUID v4 válido en formato, pero determinístico
...
function buildFiscalResidence(overrides: Partial<FiscalResidenceMin> = {}): FiscalResidenceMin {
  return {
    personType: 1,
    country: 'US',
...
function buildClientTaxData(overrides: Partial<ClientTaxData> = {}): ClientTaxData {
  const base: ClientTaxData = {
    id: 0,
    mexicoResident: true,
...
function existFiscalResidences(
  dataAutoCertification: ClientTaxData | null,
  form: FormGroup,
  dataFiscalResidencesData: WritableSignal<FiscalSelfDeclarationPageData>,
): void {
  ...
  const newTable: FiscalSelfDeclarationTableData[] = activeRows.map(
...
function isForeigner(
  ...
  if (foreignerWithoutCurpFlag) {
    ...
    form.patchValue({
      ...
      taxPostalCode: dataAutoCertification?.taxPostalCode ?? form.get('taxPostalCode')?.value,
    });
...
describe('AutoCertificationSectionComponent - Logic Specs (post-fixes)', () => {
  let form: FormGroup;
  let dataFiscalResidencesData: WritableSignal<FiscalSelfDeclarationPageData>;
  let tableData: WritableSignal<FiscalSelfDeclarationTableData[]>;
  ...
  beforeEach(() => {
    ...
    tableData = signal<FiscalSelfDeclarationTableData[]>([]);
  });
  ...
  describe('existFiscalResidences - reconstrucción de tabla (Bug Fix #3)', () => {
    ...
    it('usa country como proofOfAddressType (display) en la tabla', () => {
      ...
      expect((dataFiscalResidencesData().table as any[])[0].proofOfAddressType).toBe('US');
    });
    ...
    it('no truena con dataAutoCertification = null', () => {
      expect(() => existFiscalResidences(null, form, dataFiscalResidencesData)).not.toThrow();
      expect(dataFiscalResidencesData().data.length).toBe(0);
    });

    it('setea el control `fiscalResidences` a true si hay filas activas', () => {
      const data = buildClientTaxData({
        fiscalResidences: [buildFiscalResidence({ declarationFiscalResidence: true })] as any,
      });
  ...
  describe('normalización de declarationFiscalResidence', () => {
    it('normaliza "true" (string) a boolean true', () => {
      ...
      expect((dataFiscalResidencesData().data[0] as any).declarationFiscalResidence).toBe(true);
    });
    ...
    it('normaliza "false" (string) a boolean false', () => {
      ...
      expect((dataFiscalResidencesData().data[0] as any).declarationFiscalResidence).toBe(false);
    });
  ...
  describe('recuperación completa después de navegar de pestañas', () => {
    it('recupera radios y tabla correctamente', () => {
      ...
      expect((dataFiscalResidencesData().data[1] as any).declarationFiscalResidence).toBe(false);
    });
  ...
  describe('rowRadioSelected - toggle de declarationFiscalResidence', () => {
    it('marca sólo la fila seleccionada como activa', () => {
      ...
      expect((dataFiscalResidencesData().data[0] as any).declarationFiscalResidence).toBe(false);
      expect((dataFiscalResidencesData().data[1] as any).declarationFiscalResidence).toBe(false);
      expect((dataFiscalResidencesData().data[2] as any).declarationFiscalResidence).toBe(true);
  ...
  describe('eventRow DELETE - reseteo de controles (Bug Fix #4)', () => {
    function simulateDeleteByRegisterNo(
      ...
      fiscalResData: WritableSignal<FiscalSelfDeclarationPageData>,
      tData: WritableSignal<FiscalSelfDeclarationTableData[]>,
    ) {
      const curr = fiscalResData();
      const newTable = curr.table.filter((r) => r.registerNo !== deleteRegisterNo);
      const newData = curr.data.filter((_, idx) => idx !== deleteRegisterNo - 1);
      ...
      } else if (!(newData as any[]).some((r) => r.declarationFiscalResidence === true)) {
  ...
  describe('validForm - validación explícita sobre tabla (Bug Fix #4)', () => {
    function simulateValidForm(fiscalResData: WritableSignal<FiscalSelfDeclarationPageData>): {
      isValid: boolean;
      error: string;
    } {
      const rows = (fiscalResData().data as any[]).filter((d) => d.active !== false);
  ...
  describe('RFC dinámico y shortcuts', () => {
    ...
    it('XAXX010101000 → llena name, fiscalRegimeId, cfdiUse, taxPostalCode', () => {
      ...
      expect(form.get('fiscalRegimeId')?.value).toBe('SIN OBLIGACIONES FISCALES');
  ...
  describe('Regla nueva: CFDI dinámico según régimen fiscal', () => {
...
describe('AutoCertificationSectionComponent - Real Component Specs', () => {
  let component: AutoCertificationSectionComponent;
  let fixture: ComponentFixture<AutoCertificationSectionComponent>;
  ...
  let modalService: jasmine.SpyObj<ModalFormService>;
  ...
  beforeEach(async () => {
    ...
    notificationModalService = jasmine.createSpyObj<NotificationModalService>(
      'NotificationModalService',
      ['confirm'],
    );
    modalService = jasmine.createSpyObj<ModalFormService>('ModalFormService', [
    ...
    catalogsService.getCfdi.and.returnValue(of([{ cfdiUse: 'G03' }] as any));
    catalogsService.getFiscalRegime.and.returnValue(
      of([{ fiscalRegimeId: '601', fiscalRegime: 'General' }] as any),
    );
    notificationModalService.confirm.and.returnValue(Promise.resolve({ value: true }));
    modalService.fiscalResidenceModal.and.returnValue(of(null));

    await TestBed.configureTestingModule({
      imports: [ReactiveFormsModule],
      declarations: [AutoCertificationSectionComponent],
      providers: [
        ...
        { provide: ModalFormService, useValue: modalService },
        {
          provide: UnsavedChangesService,
          useValue: { setUnsavedChanges: jasmine.createSpy('setUnsavedChanges') },
        },
      ...
      .overrideComponent(AutoCertificationSectionComponent, {
        set: { template: '' },
      })
      .compileComponents();

    fixture = TestBed.createComponent(AutoCertificationSectionComponent);
    component = fixture.componentInstance;
  });

  it('should rebuild fiscal residences from input data on init', () => {
    component.dataAutoCertification = buildClientTaxData({
      fiscalResidences: [
        buildFiscalResidence({
          declarationFiscalResidence: 'true',
          country: 'US',
    ...
    expect(component.dataFiscalResidencesData().data.length).toBe(1);
    expect(component.dataFiscalResidencesData().data[0].declarationFiscalResidence).toBeTrue();
    expect(component.form.get('fiscalResidences')?.value).toBeTrue();
    expect(component.form.get('declarationFiscalResidence')?.value).toBeTrue();
  });

  it('should strip tempId and policy from non-maintenance submit payload', async () => {
    component.dataAutoCertification = buildClientTaxData({ fiscalResidences: [] as any });
    fixture.detectChanges();

    component.dataFiscalResidencesData.set({
      data: [
        {
          tempId: 'temp-1',
          personId: 321,
    ...
    expect(result.fiscalResidences[0].tempId).toBeUndefined();
    expect(result.fiscalResidences[0].policy).toBeUndefined();
    expect(result.fiscalResidences[0].active).toBeUndefined();
    expect(result.fiscalResidences[0].personId).toBeUndefined();
    expect(result.fiscalResidences[0].factaObligations.factaId).toBeUndefined();
  });

  it('should enable cfdiUse and refresh options when fiscal regime changes', () => {
    fixture.detectChanges();
    component.form.get('cfdiUse')?.setValue('OLD');

    component.onFiscalRegimeIdChange({ value: '601' } as any);

    expect(catalogsService.getCfdi).toHaveBeenCalledWith({
      personType: 'PF',
      fiscalRegimeId: '601',
    });
    expect(component.form.get('cfdiUse')?.enabled).toBeTrue();
    expect(component.cfdi().length).toBe(1);
    expect(component.form.get('cfdiUse')?.value).toBe('');
  });

  it('should select only one fiscal residence when row radio changes', () => {
    fixture.detectChanges();
    component.dataFiscalResidencesData.set({
      data: [
        { tempId: 'a', declarationFiscalResidence: false, active: true } as any,
        { tempId: 'b', declarationFiscalResidence: true, active: true } as any,
      ],
      table: [
        { tempId: 'a', declarationFiscalResidence: false } as any,
        { tempId: 'b', declarationFiscalResidence: true } as any,
      ],
    });

    component.rowRadioSelected({ row: { tempId: 'a' } });

    expect(component.dataFiscalResidencesData().data[0].declarationFiscalResidence).toBeTrue();
    expect(component.dataFiscalResidencesData().data[1].declarationFiscalResidence).toBeFalse();
    expect((component.tableData()[0] as any).declarationFiscalResidence).toBeTrue();
    expect(component.form.get('declarationFiscalResidence')?.value).toBeTrue();
  });

  it('should add a fiscal residence from modal result and mark first row as active', () => {
    component.dataAutoCertification = buildClientTaxData({ fiscalResidences: [] as any });
    modalService.fiscalResidenceModal.and.returnValue(
      of({
        fiscalResidences: [
          {
            personId: 10,
  ...
  it('should delete the last fiscal residence and reset summary flags', async () => {
    fixture.detectChanges();
    component.dataFiscalResidencesData.set({
      data: [
        {
          ...
          active: true,
          declarationFiscalResidence: true,
          factaObligations: { factaId: null },
        } as any,
      ],
      table: [{ tempId: 'temp-1', declarationFiscalResidence: true } as any],
    });
    component.tableData.set([{ tempId: 'temp-1', declarationFiscalResidence: true } as any]);
    component.form.patchValue({
      fiscalResidences: true,
      declarationFiscalResidence: true,
      fatca: true,
    ...
    component.eventRow({ type: STRINGS.DELETE, row: { tempId: 'temp-1' } });
    await fixture.whenStable();

    expect(component.dataFiscalResidencesData().data.length).toBe(0);
    expect(component.tableData().length).toBe(0);
    expect(component.form.get('fiscalResidences')?.value).toBeFalse();
    expect(component.form.get('declarationFiscalResidence')?.value).toBeFalse();
    expect(component.form.get('fatca')?.value).toBeFalse();
    expect(component.form.get('crs')?.value).toBeFalse();
  });
});
