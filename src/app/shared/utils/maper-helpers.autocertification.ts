export function normalizeBoolean(value: any, defaultValue?: boolean): boolean {
...
export function proofOfAddressTypeIdToDesc(id?: string): string {
  const v = String(id ?? '')
    ...
    .toUpperCase();
  return (
    PROOF_OF_ADDRESS_TYPE_MAP.find((x) => x.proffOfAddressTypeId === v)?.desProffOfAddressType ?? ''
  );
}
...
export function autenticationIdToDesc(id?: string): string {
  const v = String(id ?? '')
    ...
    .toUpperCase();
  return (
    AUTENTICATION_TYPE_MAP.find((x) => x.autenticationTypeId === v)?.desAutenticationType ?? ''
  );
}
