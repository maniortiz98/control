...
import { firstValueFrom, lastValueFrom, map, Observable, startWith } from 'rxjs';
...
import { RealOwner, RealOwnerData } from '../../models/real-owner';
import { RealOwnerService } from '../../../shared/services/storage-services/real-owner.service';
import { AllowedValuesRfcNifTinNss, compareAndReturnIdRfcNifTinNss, compareAndReturnValueRfcNifTinNss } from '../../../shared/utils/map-rfc-nif-tin-nss';
import { convertDateBack, formatDateYYYYMMDD, yearsAgo } from '../../../shared/utils/datetime';
import { validateRFCMonth, validateRFCDay } from '../../../shared/utils/rfcValid';
import { mapToCheckPointRealOwner, mapToCheckPointRealOwnerMant } from '../../services/mappers/real-owner-maper';
...
import { formFunctionEnAll, butonFunctionDis, buttonFunctionEn, formFunctionDis, formFunctionDisMatch } from '../../../shared/utils/disableOrEnabled';
...
@Component({
  selector: 'app-real-owner',
  standalone: false,
  templateUrl: './real-owner.component.html',
  styleUrl: './real-owner.component.scss'
})
export class RealOwnerComponent {
  ...
  //Constructor
  constructor() {
    this.personType.set(this.onboardingService.getCurrentInfo().personType);
  ...
  ngOnInit() {
    this.data = this.realOwnerService.getItem();
    this.economicActivity.set(this.route.snapshot.data['economicActivityResolver']);
    this.filteredEconomicActivities.set(this.route.snapshot.data['economicActivityResolver']);
    const bbRel = {
      bool: '',
      clientId: '',
      language: '',
    ...
    if (this.isMaintenance) {
      this.mant = {
        ...
        fieldsDisabled: [],
        fieldsEnabled: [],
        butonsDisabled: this.permissionRolService.getPermissions()['real-owner'].buttonsDisabled
      }
    }
  }
  ngAfterViewInit() {
    ...
    if (this.data && this.isNotEmpty(this.data)) {
      ...
      this.ppeSectionComponent.setDataRealOwnerPPE(this.data.ppe);
    }
    if (this.isMaintenance) {
      console.log('entra pr')
      ...
      if (!this.permissionRolService.getPermissions()['real-owner'].allDisabled) {
  ...
  editt() {
    console.log(this.permissionRolService.getPermissions()['real-owner'].allDisabled)
    if (this.permissionRolService.getPermissions()['real-owner'].allDisabled) {
    } else {
      if (this.data?.generalData.foreignerWithoutCurp === true) {
      ...
      butonFunctionDis(this.permissionRolService.getPermissions()['real-owner'].buttonsDisabled);
      this.mant = {
        isMainten: this.isMaintenance,
        allDisabled: false,
        config: {
      ...
      buttonFunctionEn(this.permissionRolService.getPermissions()['real-owner'].buttonsEnabled);
  ...
  async onSubmit() {
    ...
    if (ppeSection != null && generalData != null && addresData != null && !this.validadorForm()) {
      ...
      if (!this.isMaintenance) {
        this.checkpointService.saveSection('real-owner', mapToCheckPointRealOwner(data)).subscribe((result) => {
          if (result['status'] === "CREATED") {
            this.realOwnerService.setItem(data);
        ...
        this.checkpointService.saveSectionMant('real-owner', mapToCheckPointRealOwnerMant(data, this.realOwnerService.getItemCopy())).subscribe(async (result) => {
          if (result['status'] === "CREATED") {
            await this.update();
            this.unsavedChangesService.setUnsavedChanges(false);
            this.notificationService.success('Guardado con éxito');
 ...
 getDateOfBirthFromCURP(curp: string): Date | null {
  ...
  validador(): boolean {
    ...
    if (
      ...
      || this.profileForm.getRawValue().typePerson?.trim() === ''
    ) {
      this.error();
  ...
  getDataClient(): DataClient {
    const data: RealOwner = this.client();
    ...
    if (!this.validador()) {
      ...
      if (respHom.passOnHomonyms) {
        ...
        this.customerInformationService.getCustomerInfo(respHom.numberClient ?? 0).subscribe(async data => {
          this.setClientData(this.mapToRealOnwner(data));
  ...
  setClientData(data: RealOwnerData) {
    const countryTaxCodeAbroad = this.profileForm.get('countryTaxCodeAbroad');
    if (data) {
  ...
  mapToRealOnwner(data: CustomerInformation): RealOwnerData {
    return {
      generalData: {
        ...
        expirationDateField: formatDateYYYYMMDD(data.generalInformation?.fielExpirationDate ?? ''),
      },
      ppe: {
  ...
  client = (): RealOwner => this.profileForm.getRawValue() as RealOwner;
