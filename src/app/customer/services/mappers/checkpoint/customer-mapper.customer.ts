...
import { CustomerAdditionalInfoData, CustomerAdditionalInfoPageData, CustomerAdditionalInfoTableData } from '../../../models/customer-additional-info';
...
export function mapFormToInitialData(
  ...
  if (
    form.curp.substring(11, 13) === CustomerSTRINGS.FOREIGN ||
...
export function mapToCheckPointInitialData(form: DataClient, isMaintenance: boolean, id: number): Data {
  ...
  if (isMaintenance) {
    if (form.curp.substring(11, 13) === CustomerSTRINGS.FOREIGN) {
      return {
        curp: form.curp,
...
export function mapToClient(initialData: Data): CustomerClient {
  let dataClient: CustomerClient;

  if (
    initialData.curp.substring(11, 13) === CustomerSTRINGS.FOREIGN ||
...
export function mapFormToAdditionalInfo(
  input: CustomerAdditionalInfoData, inputTable?: CustomerAdditionalInfoTableData[],
): CustomerAdditionalInfoPageData {
  return {
    data: {
