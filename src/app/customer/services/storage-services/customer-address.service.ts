...
export class CustomerAddressService {
  ...
  private dataAddress = signal<CustomerAddress[]>([]);

  // Method to get all objects
  getAll(): CustomerAddress[] {
    return this.dataAddress();
  }

  // Method to add a new object
  validCP(data: CustomerAddress): boolean {
  ...
  // Method to add a new object
  add(data: CustomerAddress): boolean {
    const currentData = this.dataAddress();
  ...
  update(id: string, updatedData: CustomerAddress): boolean {
    const currentData = this.dataAddress();
  ...
  // Method to add a new object
  addPm(data: CustomerAddress): boolean {
    const currentData = this.dataAddress();
  ...
  // Method to update an existing object
  updatePm(id: string, updatedData: CustomerAddress): boolean {
    const currentData = this.dataAddress();
  ...
  clear(): boolean {
    ...
    if (currentData.length > 0) {
      this.dataAddress = signal<CustomerAddress[]>([]);
      return true;
    }
    return false;
  }
...
export type AddressService = CustomerAddressService;
