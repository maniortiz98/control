...
import { CustomerIdentificationItem, IdentificationItem } from '../../../models/customer-identification-item';
...
import { convertDate, convertDateBack } from '../../../utils/customer-datetime';
import { RolePermises } from '../../../../core/services/matrix_role';



@Component({
  selector: 'app-customer-identification-section',
  standalone: false,
  templateUrl: './customer-identification-section.component.html',
  styleUrl: './customer-identification-section.component.css'
})


export class CustomerIdentificationSectionComponent implements OnInit {

  identificationSaved = input<CustomerIdentificationItem[]>([]);
  ...
  identificationList = signal<CustomerIdentificationItem[]>([]);
  identificationColumns: Array<ColumnsDataTable> = [];
  identificationConfigs: ConfigDataTable = { showPag: false, showEditAction: true, showDeleteAction: true, showViewAction: false, multipleSelection: false, idName: 'id', isSelected: false};
  cantSave: boolean = false;

  today: string = '';
  countries = signal<Array<CustomerCountries>>([]);
  identificationTypes = signal<Array<CustomerIdentificationType>>([]);

  constructor(
    private readonly notificationService: CustomerNotificationsService,
  ) {
    effect(() => {
  ...
  ngOnInit() {
    ...
    this.identificationColumns = [
      { name: 'identificationCountry', title: 'País Emisor de la Identificación', show: true, type: '' },
      { name: 'identificationType', title: 'Tipo de Identificación', show: true, type: '' },
      { name: 'identificationNumber', title: 'Número de Identificación', show: true, type: '' },
      { name: 'identificationExpDate', title: 'Fecha de Vencimiento', show: true, type: '' }
    ]
  ...
  onSubmit(): void {
    if (this.form.valid) {
      ...
      const newItem: CustomerIdentificationItem = {
        ...this.form.value,
  ...
  validateExpirationDate(item: CustomerIdentificationItem): boolean {
  ...
  editIdentification(event: any) {
    const item: CustomerIdentificationItem = event.row;
