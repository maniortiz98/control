...
import { CustomerREGEX } from '../../../constants/customer-constants';

// Utils
import {
  buildCleanPayload,
  pruneEmpty,
} from '../../../utils/customer-auto-utils';
import { norm } from '../../../utils/customer-policy';
import { maxDateValidator, oneOfRequiredFlagged } from '../../../utils/customer-validators';
import { convertDateBack, toDate } from '../../../utils/customer-datetime';
...
@Component({
  selector: 'app-customer-modal-fiscal-residence',
  standalone: false,
...
export class CustomerModalFiscalResidenceComponent {
  ...
  form: FormGroup = this.fb.group({
    ...
    doubleTaxCountry: ['-'],
    tributaMX: [''],
    nif: ['', [Validators.pattern(CustomerREGEX.NIF_TIN_NSS_VALIDATION)]],
    tin: ['', [Validators.pattern(CustomerREGEX.NIF_TIN_NSS_VALIDATION)]],
    nss: ['', [Validators.pattern(CustomerREGEX.NIF_TIN_NSS_VALIDATION)]],
    factaId: [''],
    active: [true],
    personId: [''],
  ...
  private setProofValidatorsMode(mode: 'constancia' | 'idExtranjero') {
    ...
    if (mode === 'constancia') {
      ...
      if (!expSt?.value || `${expSt.value}`.trim() === '') {
        expSt?.setValue('VIGENTE', { emitEvent: false });
      }
  ...
  private applyMexicoMinimalRequiredAndClean(): void {
    ...
    const personTypeDefault = 'TITULAR';

    if (!personTypeCtrl?.value || `${personTypeCtrl.value}`.trim() === '') {
      personTypeCtrl?.setValue(personTypeDefault, { emitEvent: false });
    }
    if (!taxCountryCtrl?.value || `${taxCountryCtrl.value}`.trim() === '') {
    ...
    taxCountryCtrl?.enable({ emitEvent: false });
    ...
    ['issueDate', 'expirationDate', 'certificationDate'].forEach((n) => {
      const c = this.form.get(n);
      if (!c) return;
      c.clearValidators();
      c.setValue(null, { emitEvent: false });
    ...
    const expSt = this.form.get('expirationStatus');
    expSt?.clearValidators();
    expSt?.setValue('', { emitEvent: false });
    expSt?.disable({ emitEvent: false });
    expSt?.updateValueAndValidity({ emitEvent: false });
  }
  ...
  async saveFiscalResidenceAddress() {
    ...
    const data: CustomerFiscalSelfDeclaration = {
      mexicoResident: true,
      curp: '',
    ...
    this.form.patchValue(
      {
        ...
        personId: data.personId ?? table.personId ?? this.form.value.personId ?? null,
        ...
        factaId: table.factaId ?? data?.factaObligations?.factaId ?? null,
      },
      { emitEvent: false },
    );
