...
import { ColumnsDataTable, ConfigDataTable, CustomerPagConfig } from '../../models/customer-table-interfaces';
import { StrTempId } from '../../utils/customer-string';

@Component({
  selector: 'app-customer-table-results',
  standalone: false,
  templateUrl: './customer-table-results.component.html',
  styleUrl: './customer-table-results.component.scss',
})
export class TableResultsComponent implements OnInit, AfterViewInit, OnChanges {
   ...
   */
  protected configPag: CustomerPagConfig = {
    totalRows           : 0,
    currentPage         : 0,
    pageSize            : 5,
    disabled            : false,
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if ( changes['config'] || changes['columns'] ) {
      ...
      if ( changes['config']['currentValue']['showPag'] && changes['config']['currentValue']['showPag']['pagConfig']) {
        this.configPag = changes['config']['currentValue']['showPag']['pagConfig'];
